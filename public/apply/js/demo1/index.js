...
preloadImages('.item__img-inner').then(() => {
    ...
    // Wire up Apply button to use Lenis smooth scroll
    document.querySelectorAll('a[href="#apply-form"]').forEach(btn => {
        btn.addEventListener('click', e => {
            e.preventDefault();
            const target = document.getElementById('apply-form');
            if (target) lenis.scrollTo(target, { duration: 3, offset: -120, easing: t => t < 0.5 ? 4*t*t*t : 1-Math.pow(-2*t+2,3)/2 });
        });
    });

    // Override Lenis scroll limit to end right below the last title
    const lastTitle = document.querySelector('.content__title:last-of-type');
    if (lastTitle) {
        const max = lastTitle.offsetTop + lastTitle.offsetHeight - window.innerHeight + 150;
        Object.defineProperty(lenis, 'limit', { get: () => max });
    }
});