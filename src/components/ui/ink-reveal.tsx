...
interface InkRevealProps {
  ...
  overlayText?: string;
  overlayTextStyle?: { fontSize?: string; fontFamily?: string; fontWeight?: string; color?: string };
}
...
export default function InkReveal({
  ...
  const resize = useCallback(() => {
    ...
    if (overlayText) {
      ctx.fillStyle = overlayTextStyle.color ?? "rgba(255,255,255,0.9)";
      ctx.font = `${overlayTextStyle.fontWeight ?? "700"} ${overlayTextStyle.fontSize ?? "72px"} ${overlayTextStyle.fontFamily ?? "Arial Black, sans-serif"}`;
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText(overlayText, w / 2, h / 2);
    }
  }, [mc, overlayText, overlayTextStyle]);
